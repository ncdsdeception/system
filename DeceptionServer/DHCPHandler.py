...
class DHCPHandler(object):
    ...
    def handleDiscover(self,pkt):
        ...
        for node in self.networkview.access:
            print("Node " + str(node.eth_addr))

        eth_server = "00:00:00:00:00:01"
        ...
        bootp = BOOTP(op=2,yiaddr=n.ip_addr,siaddr="0.0.0.0",giaddr=gateway,chaddr=binascii.unhexlify(pkt[0][Ether].src.replace(":","")),xid=pkt[0][BOOTP].xid)
